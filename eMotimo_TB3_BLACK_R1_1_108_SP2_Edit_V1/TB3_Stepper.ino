...
void do_step(byte step_pin) //optimize this later to hardcode actual pins
{
...
long calculate_feedrate_delay_video()
{
  ...
  if (program_progress_2PT == 3) current_feedrate = ((interval * (1000L) * long(keyframe[0][3] - keyframe[0][2])) / master_steps); //  total move for all linear
  else                           current_feedrate = ((interval * (1000L)) / master_steps); //  Use the full time for video - hardcoded to 1000 *50 mc or 50000us or 0.050 seconds or 20hz
...
void Turn_on_ISRTimer1 ()
{
  TCCR1A = 0;
  TCCR1B = _BV(WGM13);

  ICR1 = (F_CPU / 4000000) * TIME_CHUNK; // goes twice as often as time chunk, but every other event turns off pins 16000000/4000000=4  4*50=200, we have 200 clock cycles to complete our ISR
  TCCR1B &= ~(_BV(CS10) | _BV(CS11) | _BV(CS12));
  TIMSK1 = _BV(TOIE1);
  TCCR1B |= _BV(CS10);
}

void Turn_off_ISRTimer1 ()
{
  TIMSK1 &= ~(_BV(TOIE1)); //clears the Timer1bit
}