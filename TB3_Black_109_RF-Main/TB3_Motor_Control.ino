...
void move_motors()
{
	...
	if ((progtype==REG2POINTMOVE || progtype==REV2POINTMOVE ||progtype==AUXDISTANCE) && (intval==VIDEO_INTVAL))
...
void go_to_start_new() // interrupt routine
{
  ...
  Serial.println("motors[0].dest:" + String(motors[0].destination));
	...
	if (POWERSAVE_PT > 2  && sequence_repeat_type) disable_PT(); //don't powersave for continuous
	if (POWERSAVE_AUX > 2 && sequence_repeat_type) disable_AUX(); //don't powersave for continuous
}
