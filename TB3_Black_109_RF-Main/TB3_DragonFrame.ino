  ...
  static bool toggleStep = 0;
	...
	else
	{
		...
		if (velocityUpdateCounter == VELOCITY_UPDATE_RATE)
		{
			...
			for (uint8_t i = 0; i < MOTOR_COUNT; i++)
			{
				if (*motorMoveSpeed[i] && !motors[i].nextMotorMoveSpeed)	bitSet(sendPosition, i);
...
void setPulsesPerSecond(uint8_t motorIndex, uint16_t pulsesPerSecond)
{
	...
	motors[motorIndex].maxAcceleration = pulsesPerSecond / 2;  
}


void setupMotorMove(uint8_t motorIndex, int32_t destination)
{
...
void stopMotor(uint8_t motorIndex)
{
	...
	uint32_t v = 20000 * motors[motorIndex].nextMotorMoveSpeed / 65536;
	uint32_t maxA = motor->maxAcceleration;
	uint32_t maxV = motor->maxVelocity;
...
boolean isValidMotor(uint8_t motorIndex)
{
...
{
	uint8_t avail = SERIAL_DEVICE.available();
	uint8_t motor;

	for (uint8_t i = 0; i < avail; i++)
	{
		byte cmd = processUserMessage(SERIAL_DEVICE.read());
		...
		if (cmd != CMD_NONE)
		{
			...
			switch (cmd)
			{
				...
				case CMD_BF:
					...
					{
						goMoDelayTime = 500;
						uint8_t motorCount = (userCmd.argCount - 2) / 4;
...
boolean jogMotor(uint8_t motorIndex, int32_t target, int32_t * destination)
{
	Motor *motor = &motors[motorIndex];
	...
	int32_t maxVelocity = motor->maxVelocity;
	int32_t maxAcceleration = motor->maxAcceleration;
	...
	if (abs(vi) < maxVelocity)
	{
		accelTime = (maxVelocity - abs(vi)) / maxAcceleration;

		if (accelTime < 0.5f)	atMaxVelocityTime = 0.5f - accelTime;
		else					accelTime = 0.5f;
	}
	else						atMaxVelocityTime = 0.5f;
	float maxVelocityReached = abs(vi) + maxAcceleration * accelTime;

	int32_t delta = abs(vi) * accelTime + (0.5f * maxAcceleration * accelTime * accelTime);
...
void inchMotor(uint8_t motorIndex, int32_t target)
{
	Motor *motor = &motors[motorIndex];
	// ideally send motor to distance where decel happens after 2 seconds

	// if switching direction, just stop
	int8_t dir = (target > motor->destination) ? 1 : -1;
...
void calculatePointToPoint(uint8_t motorIndex, int32_t destination)
{
	Motor *motor = &motors[motorIndex];
	...
	{
		int32_t vi = (motor->dir ? 1 : -1) * 20000 * motor->nextMotorMoveSpeed / 65535;
		float ti = abs(vi / motor->maxAcceleration);
		float di = motor->maxAcceleration * ti * ti / 2;
...
void setupBlur(uint8_t motorIndex, uint16_t exposure, uint16_t blur, int32_t p0, int32_t p1, int32_t p2)
{
	Motor *motor = &motors[motorIndex];
...
void synched3PtMove_max(int32_t xtarget, int32_t ytarget, int32_t ztarget) //
{
	...
	for (uint8_t mot = 0; mot < 3; mot++)
	{
		...
		DisplayMove(mot);
		if (motors[mot].destination!=motors[mot].position) bitSet(FLAGS.motorMoving, mot);
...
void calculatePointToPoint_move(uint8_t motorIndex)
{
	Motor *motor = &motors[motorIndex];
